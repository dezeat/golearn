...
- Merge method: **squash** a small single-purpose PR; **merge or
  rebase-and-merge** an epic/story integration PR so per-ticket history
  survives on `main`. The `pr` skill carries the mechanics.

## Workflow

1. On a fresh clone: `make build` (and `make hooks` if the repo provides
   git-hook installation).
2. **Identify the active unit of work.** Epics / stories / tickets live as
...
4. Read the relevant skill in `.claude/skills/` before an established
   workflow: `grill-me-with-docs` before design work, `handover` at session
   end, `parallel` for worktree-based parallel sessions, `pr` when opening a
   PR.
...
8. **Branching depth is dynamic — match it to the work.** Default is trunk:
   ...
   one coherent unit (a multi-ticket story or epic) cut an **integration
...
- **Hierarchy is native sub-issues**, never body checklists. Labels carry
  only **type** (`epic`/`story`/`ticket`) and area — not structure.
- **What stays in the repo vs GitHub** is decided by one question: _does an
  ...
  on github.com, in the agent's context): `CLAUDE.md`, `docs/architecture.md`,
  `docs/DECISIONS.md`, `.claude/skills/`. On GitHub (coordination /
...
- **GitHub Issues + Project board** — the PM hierarchy (epics / stories /
  tickets, as native sub-issues) and its live status; the active issue is a
...
- **.claude/skills/** — workflow skills: `grill-me-with-docs`, `handover`,
  `parallel`, `pr`.
