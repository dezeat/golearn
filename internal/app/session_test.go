...
func TestStartSessionWithConfig_RandomModeIgnoresPriorAttempts(t *testing.T) {
	userRepo, topicRepo, questionRepo, sessionRepo, attemptRepo, _, localUserID := setupTestDB(t)

	alice, err := userRepo.Create("alice-random", "Alice Random")
	if err != nil {
		t.Fatalf("create second user: %v", err)
	}

	// Seed attempt history only for the local user.
	seedEngine := app.NewSessionEngine(
		topicRepo,
		questionRepo,
		sessionRepo,
		attemptRepo,
		app.NewUserContext(localUserID),
		rand.New(rand.NewSource(7)),
	)
	if _, err := seedEngine.StartSession("test-topic", 3, "practice"); err != nil {
		t.Fatalf("seed StartSession: %v", err)
	}
	for {
		sq := seedEngine.GetNextSessionQuestion()
		if sq == nil {
			break
		}
		q := sq.Question
		if _, err := seedEngine.RecordAttempt(q.ID, []string{"B"}, false, 10); err != nil {
			t.Fatalf("seed RecordAttempt: %v", err)
		}
	}
	if err := seedEngine.EndSession(); err != nil {
		t.Fatalf("seed EndSession: %v", err)
	}

	randomOrder := func(userID int64) []int64 {
		...
		_, err := engine.StartSessionWithConfig(app.SessionConfig{
			TopicSlug: "test-topic",
			N:         3,
			Mode:      app.ModeRandom,
		})
		if err != nil {
			t.Fatalf("random StartSessionWithConfig: %v", err)
		}

		if engine.ActiveMode() != app.ModeRandom {
			t.Fatalf("expected active mode random, got %s", engine.ActiveMode())
		}
		...
		if err := engine.EndSession(); err != nil {
			t.Fatalf("random EndSession: %v", err)
		}
		return ids
	}

	localOrder := randomOrder(localUserID)
	aliceOrder := randomOrder(alice.ID)

	if !reflect.DeepEqual(localOrder, aliceOrder) {
		t.Fatalf("expected identical random order independent of prior attempts; got %v vs %v", localOrder, aliceOrder)
	}
}
