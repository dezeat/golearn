...
func TestSelectRandomQuestions_DeterministicWithSeed(t *testing.T) {
	questions := []domain.Question{
		makeQuestion(1),
		makeQuestion(2),
		makeQuestion(3),
	...
	s1 := SelectRandomQuestions(questions, 5, rng1)
	s2 := SelectRandomQuestions(questions, 5, rng2)
...
func TestSelectRandomQuestions_CappedAndNoDuplicates(t *testing.T) {
	questions := []domain.Question{
		makeQuestion(1),
		makeQuestion(2),
		makeQuestion(3),
	...
	selected := SelectRandomQuestions(questions, 3, rng)
	if len(selected) != 3 {
		t.Fatalf("expected 3 selected, got %d", len(selected))
	}
	...
	for _, q := range selected {
		if seen[q.ID] {
			t.Fatalf("duplicate question ID %d", q.ID)
		}
		seen[q.ID] = true
	}
}
