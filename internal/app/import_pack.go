...
func (s *ImportService) importFile(path string, result *ImportResult) (*ImportResult, error) {
	...
	return s.processPack(pack, path, result)
}

// ImportFromFS imports all YAML pack files from an [fs.FS] (e.g. an embedded
// file system). This is the same pipeline as [Import] but reads from the
// provided FS instead of the host filesystem.
func (s *ImportService) ImportFromFS(fsys fs.FS) (*ImportResult, error) {
	entries, err := fs.ReadDir(fsys, ".")
	if err != nil {
		return nil, fmt.Errorf("read embedded pack directory: %w", err)
	}
	...
	for _, entry := range entries {
		...
		pack, parseErr := s.reader.ReadPackFromBytes(data, name)
		if parseErr != nil {
		...
		result.FilesProcessed++
		if _, processErr := s.processPack(pack, name, result); processErr != nil {
...
func (s *ImportService) processPack(pack *domain.Pack, sourceName string, result *ImportResult) (*ImportResult, error) {
	...
	// Validate.
	validationErrors := domain.ValidatePack(pack, sourceName)
	if len(validationErrors) > 0 {
		for _, ve := range validationErrors {
			result.Errors = append(result.Errors, ve.Error())
		}
		result.Invalid += len(validationErrors)
		return result, fmt.Errorf("validation failed for %s (%d errors)", sourceName, len(validationErrors))
	}
