...
func TestIntroScreen(t *testing.T) {
	...
	if !containsSubstring(view, "Start") {
		t.Error("expected Start option in profile menu")
	}
}
...
func TestHomeMenuView(t *testing.T) {
	...
	if !containsSubstring(view, "Practice") {
		t.Error("expected Practice option")
	}
...
func TestSessionConfigLayoutStableAcrossModes(t *testing.T) {
	base := model{
		screen: screenSessionConfig,
		width:  120,
		selectedTopic: topicInfo{
			Topic:         domain.Topic{Name: "Go Basics"},
			QuestionCount: 15,
		},
		questionCount:      1,
		sessionConfigField: 1,
		sessionDifficulty:  "medium",
		sessionWeakestSub:  app.WeakestByQuestion,
	}

	balanced := base
	balanced.sessionMode = app.ModeBalanced
	bv := balanced.viewSessionConfig()

	weakest := base
	weakest.sessionMode = app.ModeWeakest
	wv := weakest.viewSessionConfig()

	bq := indexOf(bv, "Questions")
	wq := indexOf(wv, "Questions")
	if bq < 0 || wq < 0 {
		t.Fatalf("expected Questions row in both views")
	}
	if bq != wq {
		t.Fatalf("expected stable Questions row anchor, got balanced=%d weakest=%d", bq, wq)
	}

	bm := indexOf(bv, "Mode")
	wm := indexOf(wv, "Mode")
	if bm < 0 || wm < 0 {
		t.Fatalf("expected Mode row in both views")
	}
	if bm != wm {
		t.Fatalf("expected stable Mode row anchor, got balanced=%d weakest=%d", bm, wm)
	}
}

func TestSessionConfigColonAnchoredAtTerminalCenter(t *testing.T) {
	m := model{
		screen: screenSessionConfig,
		width:  120,
		selectedTopic: topicInfo{
		...
		questionCount:      10,
		sessionConfigField: 1,
		sessionMode:        app.ModeWeakest,
		sessionWeakestSub:  app.WeakestByQuestion,
		sessionDifficulty:  "medium",
	}

	view := m.viewSessionConfig()
	lines := splitLines(view)

	questionsColonCol := -1
	modeColonCol := -1
	for _, line := range lines {
		if questionsColonCol < 0 && containsSubstring(line, "Questions") {
			if idx := strings.Index(line, ":"); idx >= 0 {
				questionsColonCol = lipgloss.Width(line[:idx])
			}
		}
		if modeColonCol < 0 && containsSubstring(line, "Mode") {
			if idx := strings.Index(line, ":"); idx >= 0 {
				modeColonCol = lipgloss.Width(line[:idx])
			}
		}
	}

	if questionsColonCol < 0 || modeColonCol < 0 {
		t.Fatalf("expected Questions and Mode rows with ':'")
	}

	wantCenter := m.width / 2
	if questionsColonCol != wantCenter {
		t.Fatalf("Questions ':' column = %d, want %d", questionsColonCol, wantCenter)
	}
	if modeColonCol != wantCenter {
		t.Fatalf("Mode ':' column = %d, want %d", modeColonCol, wantCenter)
	}
}
