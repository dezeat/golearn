...
import (
	...
	bundledpacks "github.com/dezeat/golearn/packs"
...
func ensureBundledPacks(topicRepo *sqlite.TopicRepo, questionRepo *sqlite.QuestionRepo) error {
	...
	// Always import from the embedded FS — packs are baked into the binary so
	// the bootstrap works without any external files present.
	if _, importErr := svc.ImportFromFS(bundledpacks.FS); importErr != nil {
		return fmt.Errorf("bootstrap bundled packs from embedded FS: %w", importErr)
	}
