...
| --- | --- | --- |
| core binary size | D-015 promises an unchanged offline product | **12.2 MiB** (12,806,199 B) |
...
| migration time, populated database | D-014 migrations must not stall startup | **0.9 ms** median over three runs, 5,000 questions — the schema guard plus the migration check, on an already-migrated database. Four orders of magnitude below anything a user could perceive at startup |
